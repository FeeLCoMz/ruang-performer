...
async function readJson(req) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    export default async function handler(req, res) {
      ...
      const url = req.url || '';
      if (url.startsWith('/song-search')) {
        return await handleSongSearch(req, res);
      }
      if (url.startsWith('/batch-search')) {
        return await handleBatchSearch(req, res);
      }
      if (url.startsWith('/transcribe')) {
        return await handleTranscribe(req, res);
      }
      // Default: chat/general AI
